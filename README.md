...
- Optionally injects `<script async type="text/javascript" src="https://cloud.wordlift.io/app/bootstrap.js"></script>` into `wp_head`.
  - consent gate filter: `wordlift_cloud_has_bootstrap_consent` (defaults to `true`)
- FAQ rendering configuration:
  - setting: `Settings > WordLift Cloud > Enable FAQ rendering` (disabled by default)
  - setting: `Settings > WordLift Cloud > FAQ target element ID`
  - setting: `Settings > WordLift Cloud > FAQ HTML template`
  - when enabled, the plugin adds FAQ-related `data-*` attributes to `bootstrap.js` and outputs the in-page `<script type="text/template" id="wl-faq-template">`
  - per-post override metabox on public post types: `Inherit (default)`, `Enable`, `Disable`
  - target container is not auto-generated by the plugin (it must exist in theme/page templates)
  - if enabled and target container is missing, logged-in admins see a frontend notice
